...
import { Person, dbCollections } from '../../../Database/tables';
...
export default function PopupFaculty({ Type, onClose, Id }) {
    ...
    const SaveData = async () => {
        ...
        if (Type === 'Edit') {
            await UpdateData(dbCollections.Person, Id, objPerson);

            showMsg(eMsg.Success, 'Faculty Updated Successfully.');
        }
        else {
            ...
            await InsertData(dbCollections.Person, objPerson);

            showMsg(eMsg.Success, 'Faculty Inserted Successfully.');
        }
    ...
    useEffect(() => {
        if (Type === 'Edit') {
            const fetchData = async () => {
                try {
                    const objFaculty = await SelectDataById(dbCollections.Person, Id);

                    if (objFaculty !== undefined && objFaculty !== null) {
                        setName(objFaculty.Name);
                        setEmail(objFaculty.Email);
                        setPhone(objFaculty.ContactNo);
                    }
                } catch (error) {
                ...
                <div className='form-group'>
                    <div className='popup-header w-full'>
                        ...
                        <div className='popup-title'>
                            <h3 className='mt-0'>{Type} Faculty</h3>
                        </div>
                    </div>
                </div>
                <div className='form-group'>
